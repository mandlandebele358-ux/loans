async function generateAndSharePDF(customerId) {
  ...
  const totalInterest = loanDetails.emiAmount * loanDetails.tenureMonths - loanDetails.principal;
  ...
  template = template.replace("{{emiAmount}}", formatCurrency(loanDetails.emiAmount));
  ...
  template = template.replace("{{totalInterest}}", formatCurrency(totalInterest));
  ...
  const emiRows = emiSchedule
    ...
    <tr class="${emi.status === 'Paid' ? 'paid' : ''}">
      ...
      <td><span class="status ${emi.status.toLowerCase()}">${emi.status}</span></td>
    </tr>
  `
    )
    .join("");
  ...
  const canvas = await html2canvas(container.querySelector(".receipt-container"));
